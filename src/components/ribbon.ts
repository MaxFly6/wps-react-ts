...
export interface RibbonCallbacks {
    OnAddinLoad(ribbonUI: WPS.RibbonUi): boolean;
    OnAction(control: RibbonControl): boolean | void;
    GetImage(control: RibbonControl): string;
    OnGetEnabled(control: RibbonControl): boolean;
    OnGetVisible(control: RibbonControl): boolean;
    OnGetLabel(control: RibbonControl): string;
    OnNewDocumentApiEvent(doc: Et.Workbook): void;
}

const ribbon: RibbonCallbacks = {
    //这个函数在整个wps加载项中是第一个执行的
    OnAddinLoad: function(ribbonUI) {
        if (typeof (window.Application.ribbonUI) != "object") {
            (window.Application as { ribbonUI: WPS.RibbonUi }).ribbonUI = ribbonUI
        }
    ...
    OnAction: function(control) {
        ...
        switch (eleId) {
            ...
            case "btnIsEnbable":
                {
                    const bFlag = Util.GetPluginStorageValue<boolean>("EnableFlag") ?? false
            ...
            case "btnApiEvent":
                {
                    const bFlag = Util.GetPluginStorageValue<boolean>("ApiEventFlag") ?? false
